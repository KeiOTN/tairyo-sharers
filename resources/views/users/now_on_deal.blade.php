...
<x-app-layout>
        ...
        <div class="p-2 w-full alert alert-success flex justify-center">
            <div class="bg-white border-gray-200 border rounded flex p-2 h-full items-center">
                <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                    class="text-blue-500 w-6 h-6 flex-shrink-0 mr-4" viewBox="0 0 24 24">
    ...
    @if (session('delete_status'))
        <div class="p-2 w-full alert alert-success flex justify-center">
            <div class="bg-white border-gray-200 border rounded flex p-2 h-full items-center">
                ...
                <span class="title-font font-medium"> {{ session('delete_status') }}</span>
            </div>
        </div>
    @endif
                    ...
                    @else
                        ...
                        @foreach ($contents as $content)
                            <div class="flex flex-row text-blue-300 text-xs items-center justify-end pr-4">
                                ...
                                <form action="{{ route('delete', ['content_id' => $content->id]) }}" method='get'
                                    class="mt-2 flex flex-row font-xs">

                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 md:h-6 md:w-6" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor" {{-- onClick="location.href='{{ route('delete', ['content_id' => $content->id]) }}'" --}}>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <script>
        ...
        function delete_alert(e) {
            ...
            // return route('delete', ['content_id' => $content - > id]);
        };
    </script>
</x-app-layout>
